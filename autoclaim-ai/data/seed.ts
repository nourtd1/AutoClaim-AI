...
function loadEnv() {
  ...
  } catch { /* .env.local not found — uses local file DB */ }
}
...
async function main() {
  ...
  const approved = await createClaim({
    ...
    extractedData:   { confidence:0.97, policyNumber:"POL-2024-001", claimantName:"Marie Dupont", incidentDate:"2024-03-10", claimAmount:850, claimType:"PROPERTY_DAMAGE", documentList:["photos_degats.pdf","devis_reparation.pdf"], rawText:"Déclaration sinistre – bris de vitre", missingFields:[] },
  ...
  await addStageEvent({ claimId:approved.id, stage:"RESOLUTION",  status:"APPROVED",  actor:"ROBOT", notes:"Approbation automatique — risque faible" });
  ...
  const rejected = await createClaim({
    ...
    documents: [
      ...
      { id:"doc-005", name:"factures_medicales.pdf",     type:"application/pdf", status:"MISSING", url:null,                  uploadedAt:"2024-04-06T10:00:00.000Z" },
    ],
    ...
    reviewNotes: "Dossier rejeté : documents obligatoires non fournis.", assignedTo: alice.id, resolvedAt: "2024-04-15T09:00:00.000Z",
  });
  await addStageEvent({ claimId:rejected.id, stage:"INTAKE",       status:"SUBMITTED",    actor:"ROBOT", notes:"Email reçu" });
  await addStageEvent({ claimId:rejected.id, stage:"EXTRACTION",   status:"EXTRACTING",   actor:"AGENT", notes:"Documents partiels détectés" });
  ...
  await addStageEvent({ claimId:rejected.id, stage:"RESOLUTION",   status:"REJECTED",     actor:"HUMAN", notes:"Rejet définitif après délai de 10 jours" });

  // 3 — PENDING_REVIEW
  const pendingReview = await createClaim({
    ...
    extractedData:   { confidence:0.88, policyNumber:"POL-2024-003", claimantName:"Sophie Leclerc", incidentDate:"2024-05-20", claimAmount:18500, claimType:"ACCIDENT", documentList:["constat_amiable.pdf","rapport_expertise.pdf","photos_accident.pdf"], rawText:"Sinistre automobile – collision frontale", missingFields:[] },
    validationResult:{ isValid:true, policyExists:true, documentsComplete:true, amountWithinLimit:false, errors:[], warnings:["Montant supérieur au seuil — revue humaine requise"], riskScore:52 },
    reviewNotes: null, assignedTo: bob.id, resolvedAt: null,
  });
  await addStageEvent({ claimId:pendingReview.id, stage:"INTAKE",       status:"SUBMITTED",    actor:"ROBOT", notes:"PDF reçu via portail" });
  await addStageEvent({ claimId:pendingReview.id, stage:"EXTRACTION",   status:"EXTRACTING",   actor:"AGENT", notes:"Extraction réussie — confiance 88%" });
  await addStageEvent({ claimId:pendingReview.id, stage:"VALIDATION",   status:"VALIDATING",   actor:"AGENT", notes:"Montant dépasse le seuil automatique" });
  await addStageEvent({ claimId:pendingReview.id, stage:"HUMAN_REVIEW", status:"PENDING_REVIEW",actor:"ROBOT",notes:"Escalade automatique vers Bob Diallo" });
  ...
  const escalated = await createClaim({
    ...
    validationResult:{ isValid:false, policyExists:true, documentsComplete:false, amountWithinLimit:false, errors:["Dépôt de plainte invalide"], warnings:["3ème sinistre vol en 18 mois — pattern suspect","Montant élevé (42 000 EUR)"], riskScore:91 },
  ...
  await addStageEvent({ claimId:escalated.id, stage:"VALIDATION",         status:"VALIDATING",actor:"AGENT", notes:"Score de risque : 91/100" });
  await addStageEvent({ claimId:escalated.id, stage:"EXCEPTION_ROUTING",  status:"ESCALATED", actor:"AGENT", notes:"Pattern fraude détecté — escalade Compliance" });

  // 5 — VALIDATING
  const validating = await createClaim({
    policyNumber: "POL-2024-005", claimantName: "Isabelle Fontaine", claimantEmail: "i.fontaine@email.com",
    claimType: "LIABILITY", incidentDate: "2024-06-10", claimAmount: 7500, currency: "EUR",
    description: "Responsabilité civile — dégâts causés chez un voisin lors de travaux.",
    status: "VALIDATING", priority: "MEDIUM", stage: "VALIDATION", source: "EMAIL",
    documents: [
      { id:"doc-011", name:"devis_voisin.pdf",   type:"application/pdf", status:"PRESENT", url:"/uploads/doc-011.pdf", uploadedAt:"2024-06-11T14:00:00.000Z" },
      { id:"doc-012", name:"attestation_rc.pdf", type:"application/pdf", status:"PRESENT", url:"/uploads/doc-012.pdf", uploadedAt:"2024-06-11T14:05:00.000Z" },
    ],
    extractedData:   { confidence:0.93, policyNumber:"POL-2024-005", claimantName:"Isabelle Fontaine", incidentDate:"2024-06-10", claimAmount:7500, claimType:"LIABILITY", documentList:["devis_voisin.pdf","attestation_rc.pdf"], rawText:"RC voisin – dégâts travaux 7 500 EUR", missingFields:[] },
    validationResult: null, reviewNotes: null, assignedTo: null, resolvedAt: null,
  });
  await addStageEvent({ claimId:validating.id, stage:"INTAKE",     status:"SUBMITTED", actor:"ROBOT", notes:"Email parsing automatique" });
  await addStageEvent({ claimId:validating.id, stage:"EXTRACTION", status:"EXTRACTING",actor:"AGENT", notes:"Extraction complète — confiance 93%" });
  await addStageEvent({ claimId:validating.id, stage:"VALIDATION", status:"VALIDATING",actor:"AGENT", notes:"Validation en cours…" });

  // 6 — SUBMITTED
  const submitted = await createClaim({
    policyNumber: "POL-2024-006", claimantName: "Thomas Bernard", claimantEmail: "t.bernard@email.com",
    ...
    documents: [
      { id:"doc-013", name:"certificat_medical.pdf", type:"application/pdf", status:"PRESENT", url:"/uploads/doc-013.pdf", uploadedAt:"2024-06-12T18:30:00.000Z" },
    ],
    extractedData: null, validationResult: null, reviewNotes: null, assignedTo: null, resolvedAt: null,
  });
  await addStageEvent({ claimId:submitted.id, stage:"INTAKE", status:"SUBMITTED", actor:"ROBOT", notes:"Formulaire reçu — en attente de traitement" });
