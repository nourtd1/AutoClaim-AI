...
const PAGE_META: Record<string, { title: string; subtitle: string; icon: React.ReactNode }> = {
  ...
  "/claims/new": {
    ...
    icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg>,
  },
  "/review": {
    title: "Review Queue",
    subtitle: "Claims awaiting human decision",
    icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>,
  },
};
...
export default function TopBar({ title, subtitle, badge, actions, pending }: TopBarProps) {
  ...
  return (
    <header className="sticky top-0 z-40"
      style={{ background:"rgba(255,255,255,0.95)", backdropFilter:"blur(12px)", borderBottom:"1px solid #E2E8F0", boxShadow:"0 1px 3px rgba(0,0,0,0.04)" }}>
      <div className="px-6 py-3 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <h1 className="text-sm font-bold tracking-tight truncate" style={{ color:"#1E293B" }}>
                {displayTitle}
              </h1>
              {badge && (
            ...
            {displaySub && (
              <p className="text-[11px] truncate mt-0.5" style={{ color:"#94A3B8" }}>{displaySub}</p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full"
            ...
            <span className="text-[10px] font-semibold uppercase tracking-widest" style={{ color:"#6366F1" }}>Live</span>
          </div>
          ...
          {pathname !== "/review" && pending !== undefined && pending > 0 && (
            <Link href="/review"
              ...
              <span className="rounded-full px-1.5 py-0.5 text-[10px] font-bold text-white"
                style={{ background:"#F97316" }}>{pending}</span>
            </Link>
          )}
