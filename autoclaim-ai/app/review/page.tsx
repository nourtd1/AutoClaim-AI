import { initDb, getDb, getAvailableReviewers, getReviewerById } from "@/lib/db";
...
async function getQueueItems(): Promise<QueueItem[]> {
  ...
  type Row = {
    ...
    reviewNotes:string|null; assignedTo:string|null; createdAt:string; updatedAt:string; resolvedAt:string|null;
  };
  ...
  const rows = res.rows.map(r => r as unknown as Row);
  ...
  return Promise.all(rows.map(async row => ({
    ...
    claimAmount: Number(row.claimAmount), currency: row.currency, description: row.description,
    status: row.status as Claim["status"], priority: row.priority as Claim["priority"],
    stage: row.stage as Claim["stage"], source: row.source as Claim["source"],
    ...
    assignedReviewer: row.assignedTo ? await getReviewerById(row.assignedTo) : null,
  })));
}

export default async function ReviewQueuePage() {
  await initDb();
  const [items, reviewers] = await Promise.all([getQueueItems(), getAvailableReviewers()]);
