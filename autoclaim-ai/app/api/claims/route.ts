...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const raw = { status: searchParams.get("status") ?? undefined, priority: searchParams.get("priority") ?? undefined, source: searchParams.get("source") ?? undefined };
    const parsed = ClaimFilterSchema.safeParse(raw);
    if (!parsed.success) return NextResponse.json({ error: parsed.error.issues[0]?.message ?? "Invalid filters", code: "VALIDATION_ERROR" }, { status: 400 });
    ...
    const claims = await getAllClaims(filters);
...
export async function POST(req: NextRequest) {
  ...
  try {
    const claim = await createClaim({ ...parsed.data, status: "SUBMITTED", priority: "MEDIUM", stage: "INTAKE", documents: [], extractedData: null, validationResult: null, reviewNotes: null, assignedTo: null, resolvedAt: null });
    await addStageEvent({ claimId: claim.id, stage: "INTAKE", status: "SUBMITTED", actor: "ROBOT", notes: `Claim received via ${parsed.data.source}` });
